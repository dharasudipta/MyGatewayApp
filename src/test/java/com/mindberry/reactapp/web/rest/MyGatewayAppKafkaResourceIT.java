package com.mindberry.reactapp.web.rest;
...
import com.mindberry.reactapp.config.KafkaProperties;
...
class MyGatewayAppKafkaResourceIT {
    ...
    void setup() {
        ...
        MyGatewayAppKafkaResource kafkaResource = new MyGatewayAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/my-gateway-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/my-gateway-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
